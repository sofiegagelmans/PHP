...
function ValidateUsrPassword(){
    if (strlen($_POST["usr_password"]) < 8){
        $msgs = "Het wachtwoord moet minstens 8 tekens bevatten.";
        $_SESSION['errors']["usr_password_error"] = $msgs;
    }
}
...
function ValidateUsrEmail() {
    ...
    if ( ! filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $msgs = "Ongeldig e-mailadres.";
        $_SESSION['errors'][ "usr_email_error" ] = $msgs;
    }
}
